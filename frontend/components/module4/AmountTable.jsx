...
const AmountTable = ({ rows = [] }) => {
    ...
    return (
        <div className="bg-gray-900 text-xs rounded-lg p-2 shadow-lg h-full  border border-purple-700/30">
            ...
            {aggregatedData.length === 0 ? (
                ...
                <div className="overflow-y-auto max-h-49 scrollbar-hide  rounded-md border border-purple-700/30">
                    <table className="min-w-fit  border border-purple-700/40  ">

                        <thead className="sticky top-0 z-10 bg-purple-900/90 text-purple-300">
                            <tr>
                        ...
                        <tbody>
                            {aggregatedData.map((row, idx) => (
                                ...
                                >
                                    <td className="px-2 py-2 text-purple-200">{row.SupplierName}</td>
                                    <td className="px-2 py-2 text-right text-white">
                                        {row.TotalQuantity.toFixed(2)}
                                    </td>
                                    <td className="px-2 py-2 text-center text-purple-300">{row.UOM}</td>
                                    <td className="px-2 py-2 text-right text-white">
